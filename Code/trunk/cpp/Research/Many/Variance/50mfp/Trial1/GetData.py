...
if __name__ == "__main__":
    ...
    AMeanSD = numpy.mean(Asd)
    ATrueSD = numpy.std(Am)
    ADiff = (AMeanSD-ATrueSD)/ATrueSD
    PMeanSD = numpy.mean(Psd)
    PTrueSD = numpy.std(Pm)
    PDiff = (PMeanSD-PTrueSD)/PTrueSD

    print "Power   & %8.5f & %6.1E & %6.1E & %6.3f \\\\" %(
            numpy.mean(Pm), PMeanSD, PTrueSD, PDiff)
    print "Arnoldi & %8.5f & %6.1E & %6.1E & %6.3f \\\\" %(
            numpy.mean(Am), AMeanSD, ATrueSD, ADiff)
