...
// Function returns a vector of Node positions.
vector<double> Mesh::NodePositions(){
    vector<double> pos;
    vector<Node>::iterator NodeIter = mNodes.begin();
    for( ; NodeIter != mNodes.end(); ++NodeIter ){
        pos.push_back(NodeIter->x());
    }
...
// Function returns a vector of Zone centers.
vector<double> Mesh::ZoneCenters(){
    vector<double> edges = NodePositions();
    vector<double>::iterator edgeIter = edges.begin();
    vector<double> centers(mNumZones);
    vector<double>::iterator centIter = centers.begin();

    double distance;
//  for( ; edgeIter != edges.end(); ++edgeIter, ++centIter ){
//      distance = (*(edgeIter + 1) - *edgeIter)/2.0;
//      *centIter = *edgeIter + distance;
//  }
    for( int i = 0; i < mNumZones; ++i ){
        distance = (edges[i+1] - edges[i])/2.0;
        centers[i] = distance + edges[i];
    }
    return centers;
}
