...
#define RCSID_EXPONENTIALPROB_H "$Id: ExponentialProb.hpp 6328 2006-10-26 12:25:48Z ckarney $"
...
namespace RandomLib {
  ...
  inline bool ExponentialProb::ExpFraction(Random& r, RealType p) const
	...
	if (w > _v[j])
	  ...
	  _v[j] = w;		// New V is smaller, update V
	  m = j + 1;		// adjusting its size
	  break;		// and generate the next V
	}
	// Else need to check the next c bits
      }
    }
  }
} // namespace RandomLib
