__id__       = "$Id: procOUTSN.py 65 2007-05-01 19:38:43Z jlconlin $"
__author__   = "$Author: jlconlin $"
__version__  = "$Revision: 65 $"
__date__     = "$Date: 2007-05-01 13:38:43 -0600 (Tue, 01 May 2007) $"
...
class procOutsn(object):
    ...
    def writeFile(self, outfile):
        ...
        data['FissionSource'] = (self.X, self.FS)
        gnuFile = gnuplotFile.gnuplotFile(outfile, data)
    ...
    def coursenSNFS(self, geo):
        ...
        mcbins = geo.bins
        centers = geo.centers
        heights = scipy.zeros(mcbins)
        
        binCombo = float(snbins)/mcbins
        for i in xrange(mcbins):
            heights[i] =scipy.stats.mean(
                self.FS[binCombo*i:binCombo*(i+1)])
    ...
    def __process(self):
        ...
        self.F1 = scipy.array(F1)
        self.F2 = scipy.array(F2)
        self.F3 = scipy.array(F3)
