__id__       = "$Id: GetData.py 718 2009-07-17 21:23:07Z jlconlin $"
...
def main():
    ...
    fileDir = os.path.join(os.getcwd(), 'MoreAgg')
    ...
    try:
        DatFiles.pop( DatFiles.index(
            os.path.join(fileDir, "50mfp.Power.dat" ) ) )
    ...
    # Get Eigenvectors
    aFilename = os.path.join(fileDir, "50mfp.r0.001.dat")
    gnuArnoldi = gnuFile.gnuFile(aFilename)
    a0 = gnuArnoldi.Data['RAM eigenvector-0-Real']
    a1 = gnuArnoldi.Data['RAM eigenvector-1-Real']
    a2 = gnuArnoldi.Data['RAM eigenvector-2-Real']
    aFOM = gnuArnoldi.Data['FOM-Arnoldi']


    # Get power method data
    pFilename = os.path.join(fileDir, "50mfp.Power.dat")
    gnuPower = gnuFile.gnuFile(pFilename)
    p0 = gnuPower.Data['Power values']
    pVector = gnuPower.Data['Power vector']
    pFOM = gnuPower.Data['FOM-Arnoldi']
    ...
    Data['Arnoldi-Vector0'] = (a0[:,0], a0[:,1])
    Data['Arnoldi-Vector1'] = (a1[:,0], a1[:,1])
    Data['Arnoldi-Vector2'] = (a2[:,0], a2[:,1])
    Data['Arnoldi-FOM'] = (aFOM[:,0], aFOM[:,1])
    Data['Power-0'] = (p0[:,0], p0[:,1], p0[:,2])
    Data['Power-Vector'] = (pVector[:,0], pVector[:,1])
    Data['Power-FOM'] = (pFOM[:,0], pFOM[:,1])
