...
\date[PhD Thesis Defense]{August 17, 2009}
...
\begin{frame}{Acknowledgements}
    \begin{itemize}
        \item James Hollowy
        \item Bill Martin, Ed Larsen, Martin Strauss
        \item Scott and Geri Conlin
        \item Annie, Brigham, Lily, Emma
        \item Trisha Conlin
    \end{itemize}
    \note{Before we get started, I wanted to thank some of those who have played an important role in my education. \ldots}
\end{frame}
...
\begin{frame}{Monte Carlo Statistics}
    \begin{itemize}
        \item Iterations before power method converges are discarded
        \begin{itemize}
            \item \emph{Inactive} iterations
        \end{itemize}
        \item Iterations after convergence are \emph{Active} iterations
        \item Calculate mean and variance of eigenvalue estimates
        \begin{align*}
            \overline{\lambda} &\equiv \frac{1}{N} \sum_{n=1}^N \lambda_n \\[0.5em]
            \sigma_{\overline{\lambda}}^2 &\equiv \frac{1}{N-1} \left(\frac{1}{N}\sum_{n=1}^N \left(\lambda_n - \overline{\lambda}\right)^2\right)
        \end{align*}
    \item $\sigma_{\overline{\lambda}}$ is a measure of statistical uncertainty of $\overline{\lambda}$
    \end{itemize}
    
    \note{While the eigenvalue converges to the fundamental, the results of the iterations are discarded.  These iterations are referred to as inactive iterations.  Once convergence has been achieved, the eigenvalue estimates are stored so that their mean and variance can be calculated.  The standard deviation is a measure of the statistical uncertainty of the calculation.}
\end{frame}
...
\begin{frame}{Relaxing Monte Carlo Arnoldi}
    ...
    \end{itemize}
    \end{column}
    \note{One could choose a different technique for reducing the number of particles in an iteration.  I chose this one.}
...
\begin{frame}{Numerical Results}
    ...
    \begin{itemize}
        \item $\nu\Sigma_f = 1.0$, $\Sigma_a = 0.2$, $\Sigma_s = 0.8$, $\Sigma_t = 1.0$
        \item 50 mfp thick slab (DR = 0.9924)
        \item 100 inactive restarts

        \vspace{1em}
        \item $\eta$ ranges from 1E8--1.0
        \item Total number of particles is the same for all values of $\eta$
    \end{itemize}
    }
    \note{I use the same cross sections as before, but this time have a 50 mfp thick slab.}
    \begin{figure}
...
\begin{frame}{Condensing Arnoldi's Method}
    \only<1>{
    \begin{itemize}
        \item Arnoldi's method does not need many iterations to converge
        \item Why not condense all particles tracked to just a few iterations?
    \end{itemize}}
    \begin{figure} \centering
